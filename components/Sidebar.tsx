...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
    ...
    {/* Footer */ }
    < div className ="px-3 pb-4 pt-3 border-t border-zinc-800/60 space-y-3">
{
    session?.user && (
        <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-zinc-900/50 border border-zinc-800/50">
            < div className ="w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center text-white shrink-0 text-xs font-bold">
...
export default function Sidebar() {
    ...
    return (
        <>
            {/* Mobile hamburger */}
            <button
                className="md:hidden fixed top-4 left-4 z-50 w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-white shadow-xl"
